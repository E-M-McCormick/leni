...
Users can also specify their own target function by defining a custom equation, and specifying a vector of parameters in `theta`. Users can also control the outcome variable name (`y.name`) as well as the name of the variable representing time (`time.name`). **Note** The choice of `time.name` cannot be a substring of any element in `theta` - for example below `time.name = "ti"` is allowed, but `time.name = "t"` would not be.
